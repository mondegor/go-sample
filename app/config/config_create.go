...
func Create(args Args) (cfg Config, err error) {
	...
	if args.WorkDir != "" {
		...
		if !path.IsAbs(cfg.FileProviders.ImageStorage2.RootDir) {
			cfg.FileProviders.ImageStorage2.RootDir = path.Join(args.WorkDir, cfg.FileProviders.ImageStorage2.RootDir)
		}
